...
@typechecker
def make_cart_planar_annulus(
    center_pos: Float[np.ndarray, "3"],
    inner_diameter: Union[float, int],
    outer_diameter: Union[float, int],
    normal_vec: Float[np.ndarray, "3"],
    num_points: int,
    plot_annulus: Optional[bool] = False,
) -> Float[np.ndarray, "3 NumPoints"]:
    """
    创建三维平面环形阵列的均匀分布笛卡尔坐标点
    
    Args:
        center_pos:     环形中心三维坐标 [x, y, z] [m]
        inner_diameter: 内环直径 [m]
        outer_diameter: 外环直径 [m] 
        normal_vec:     平面法线方向向量 [nx, ny, nz]
        num_points:     生成点数量
        plot_annulus:   是否可视化结果
    
    Returns:
        3 x num_points 笛卡尔坐标数组
    """
    ...
    # 坐标变换到指定平面
    R, b = compute_planar_transform(center_pos, normal_vec)
    annulus = R @ points + b.reshape(-1, 1)

    # 可视化
    if plot_annulus:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(annulus[0], annulus[1], annulus[2])
        ax.set_box_aspect([1,1,1])
        plt.show()

    return annulus

def compute_planar_transform(center: np.ndarray, normal: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    计算平面坐标变换矩阵
    ...
    # 计算旋转轴和角度
    rotation_axis = np.cross(default_normal, n)
    rotation_angle = np.arccos(np.dot(default_normal, n))
